...
//	{ SUMMARY : Client program for UDP sockets }
...
int main ( int argc , char const *argv[] )	{
    ...
    while ( true ) {
		...
		int bytes_sent = sendto(client_sock, file_buf, strlen(file_buf), MSG_CONFIRM, 
								(struct sockaddr*)(&serv_addr), addrlen) ;
		if ( __debug__ )	printf("\n [ %d bytes sent from client to server ]", bytes_sent) ;
    }

	int bytes_sent = sendto(client_sock, finish, strlen(finish), MSG_CONFIRM, 
							(struct sockaddr*)(&serv_addr), addrlen) ;
	if ( __debug__ )	printf("\n [ %d bytes sent from client to server ]", bytes_sent) ;

	char text_info[RECV_SIZE + 1] ;
	int bytes_recv = recvfrom(client_sock, text_info, RECV_SIZE, MSG_WAITALL, 
							  (struct sockaddr*)(&serv_addr), &addrlen) ;
