...
//	{ SUMMARY : Server program for TCP sockets }
...
int main(int argc, char const *argv[])  {
	...
	if ( listen(server_fd, 10) < 0 )	{
		printf("\n [ ERROR : Server could not be opened for listening to client connections. ]\n\n") ;
	...
	while ( true ) {
		...
		if ( new_socket < 0 ) {
			printf("\n [ ERROR : Connection request not accepted successfully. ]\n\n") ;
        	close(server_fd) ; exit(-4) ;
		}
		...
		int bytes_sent = send(new_socket, result, strlen(result) + 1, 0) ;
